...
1. `mcr.microsoft.com/devcontainers/python:3`をベースイメージとしてpull する
1. 各種パッケージをインストールする
1. ワークスペース配下の`.venv`に python の仮想環境を作成する
1. 仮想環境にhive-builder をインストールする

通常の `.devcontainer` では `.venv/bin` が PATH に追加されるため、ターミナルや VS Code のタスクからは仮想環境内の python / pip / ansible コマンドがそのまま利用されます。
Vagrant を使う場合のみ `.devcontainer-systemd` を使って systemd 有効な開発コンテナを起動します。
...
.devcontainer-systemdディレクトリを.devcontainerにリネームしてから Codespaces を再起動して下さい。
```
mv .devcontainer .devcontainer-backup
mv .devcontainer-systemd .devcontainer
```
